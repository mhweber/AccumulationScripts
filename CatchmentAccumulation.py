...
for reg in inputs:
    ...
    if not os.path.exists(Accumulation):
        ...
        for line in data[2:]:
            ...
            if FROMCOMID==0:
                UpCOMs[TOCOMID] = []
            if FROMCOMID in Ftype.keys() and Ftype[FROMCOMID] != 'Coastline':
                UpCOMs[TOCOMID].append(FROMCOMID)
            if FROMCOMID not in Ftype.keys():
                UpCOMs[TOCOMID].append(FROMCOMID)
        ...
        if zone in interVPUs.keys():
            ...
            for line in data:
                line=line.replace("\n","").split(",")
                ID = str(line[0])
                if ID in ['18267741','20734037','1861888','1862004','1844789','7227390','11764402','6018266','5093446','11795911','11795899',
                    ...
                    if accum_type == 'Categorical': #Need to modify below for different length categorical variables...
                        ...
                        throughVPUs[ID] = []
                        # get the area weighted average of both mean value and pct full to pass back to zonal stats for downstream VPU
                        throughVPUs[ID].append(float(line[1]) + float(line[4]))
                        throughVPUs[ID].append(float(line[2]) + float(line[5]))
                        throughVPUs[ID].append(((float(line[1]) * (float(line[3]) / 100)) + (float(line[4]) * (float(line[6]) / 100))) /(float(line[1]) + float(line[4]))*100)
            # Adjustments for complicated hydro-region connections
            ...
            if zone == '06':
                if throughVPUs.has_key('1862004') and accum_type == 'Continuous':
                    throughVPUs['1862004'][3] = throughVPUs['1862004'][3] - throughVPUs['1861888'][3] #Adjust Sum
                    throughVPUs['1862004'][4] = throughVPUs['1862004'][4] - throughVPUs['1861888'][4] #Adjust Count
                    throughVPUs['1862004'][0] = throughVPUs['1862004'][0] - throughVPUs['1861888'][0] #Adjust Area
                elif throughVPUs.has_key('1862004') and accum_type == 'Count':
                    throughVPUs['1862004'][0] = throughVPUs['1862004'][0] - throughVPUs['1861888'][0] #Adjust Area
                    throughVPUs['1862004'][1] = throughVPUs['1862004'][1] - throughVPUs['1861888'][1] #Adjust Count
                    throughVPUs['1862004'][2] = throughVPUs['1862004'][1] / throughVPUs['1862004'][0] #Adjust Mean
                elif throughVPUs.has_key('1862004') and accum_type == 'Categorical':
                    for index, item in enumerate(throughVPUs['1862004']):
                        throughVPUs['1862004'][index] = throughVPUs['1862004'][index] -throughVPUs['1861888'][index]
            ...
            if zone == '11':
                ...
                if toVPUs.has_key('15334480') and accum_type == 'Continuous':
                    ...
                    for index, item in enumerate(toVPUs['15334480']):
                ...
                if toVPUs.has_key('25827824') and accum_type == 'Continuous':
                    ...
                    for index, item in enumerate(toVPUs['15334480']):
                        toVPUs['25827824'][index] = toVPUs['15334480'][index] - throughVPUs['11795899'][index]
                ...
                if throughVPUs.has_key('22850037') and accum_type == 'Continuous':
                    ...
                    for index, item in enumerate(throughVPUs['22850037']):
                ...
                if throughVPUs.has_key('22850051') and accum_type == 'Continuous':
                    ...
                    for index, item in enumerate(throughVPUs['22850051']):
